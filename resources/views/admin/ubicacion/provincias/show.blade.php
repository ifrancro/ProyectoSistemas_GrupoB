    ...
    <div class="max-w-6xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-3xl font-bold text-gray-800">{{ $provincia->nombre }}</h1>
                <div class="flex space-x-2">
                    <a href="{{ route('admin.ubicacion.provincias.edit', $provincia) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-blue-50 p-4 rounded-lg">
                    <div class="text-blue-600 text-sm font-medium">ID de la Provincia</div>
                    <div class="text-2xl font-bold text-blue-800">{{ $provincia->id_provincia }}</div>
                </div>
                <div class="bg-green-50 p-4 rounded-lg">
                    <div class="text-green-600 text-sm font-medium">Departamento</div>
                    <div class="text-2xl font-bold text-green-800">{{ $provincia->departamento->nombre }}</div>
                </div>
                <div class="bg-purple-50 p-4 rounded-lg">
                    <div class="text-purple-600 text-sm font-medium">Municipios</div>
                    <div class="text-2xl font-bold text-purple-800">{{ $provincia->municipios->count() }}</div>
                </div>
            </div>
            ...
            <div class="mb-8">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">Municipios</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($provincia->municipios as $municipio)
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="font-semibold text-gray-800">{{ $municipio->nombre }}</h3>
                        ...
                        <div class="text-sm text-gray-600">
                            <div class="mb-1">
                                ...
                                @if($municipio->asientos->count() > 0)
                                    {{ $municipio->asientos->pluck('nombre')->join(', ') }}
                                @else
                                    Sin asientos
                                @endif
                            </div>
            ...
            <div class="text-center py-8 text-gray-500">
                ...
                <p>Esta provincia no tiene municipios registrados.</p>
                <a href="{{ route('admin.ubicacion.municipios.create') }}" 
                   class="text-blue-600 hover:text-blue-800 mt-2 inline-block">
                    <i class="fas fa-plus mr-2"></i>Agregar primer municipio
                </a>
            </div>
            @endif
