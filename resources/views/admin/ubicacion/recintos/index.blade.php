...
<div class="p-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.ubicacion.recintos.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
            <i class="fas fa-plus mr-2"></i>Nuevo Recinto
        </a>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $recinto->id_recinto }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $recinto->nombre }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $recinto->direccion ?? 'Sin dirección' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $recinto->asiento->nombre }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $recinto->asiento->municipio->nombre }} - {{ $recinto->asiento->municipio->provincia->nombre }} - {{ $recinto->asiento->municipio->provincia->departamento->nombre }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            <a href="{{ route('admin.ubicacion.recintos.show', $recinto) }}" 
                            ...
                            <form action="{{ route('admin.ubicacion.recintos.destroy', $recinto) }}" 
                                  method="POST" class="inline" 
                                  onsubmit="return confirm('¿Estás seguro de eliminar este recinto?')">
                                @csrf @method('DELETE')
